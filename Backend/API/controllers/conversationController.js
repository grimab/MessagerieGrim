...
exports.createConversation = async (req, res) => {
  try {
    ...
    if (!ID_User2) {
      return res
        .status(400)
        .json({ success: false, message: "ID_User2 est requis." });
    }
    ...
    if (areFriends === 0) {
      return res
        ...
        .json({
          success: false,
          message:
            "Les utilisateurs doivent être amis pour créer une conversation.",
        });
    }
    ...
    res
      ...
      .json({
        success: false,
        message: `Erreur lors de la création de la conversation : ${error.message}`,
      });
  }
};
...
exports.deleteConversation = async (req, res) => {
  try {
    ...
    if (!conversation) {
      return res
        .status(404)
        .json({ success: false, message: "Conversation non trouvée." });
    }
    ...
    res
      .status(200)
      .json({ success: true, message: "Conversation supprimée avec succès." });
  } catch (error) {
    res.status(500).json({ success: false, message: error.message });
  }
};
