...
const {
  Groupe,
  MembreGroupe,
  HistoriqueChangementMotDePasse,
  PieceJointe,
  ID_User,
} = require("../../models");
...
exports.register = async (req, res, next) => {
  try {
    ...
    const token = jwt.sign(
      { userId: user.ID_User, roleId: user.RoleID },
      SECRET_KEY,
      {
    ...
    res
      .status(201)
      .json({ success: true, message: "Inscription réussie ! Bienvenue." });
  } catch (error) {
...
exports.setStatus = async (req, res, next) => {
  ...
  try {
    ...
    if (requestingUserRole === 2205 && userId) {
      // Vérifier si l'utilisateur est un admin et si un userId est fourni
...
exports.updateUser = async (req, res, next) => {
  try {
    ...
    if (requestingUserRole === 2205 && req.body.userId) {
      // Si l'admin met à jour un autre utilisateur
    ...
    if (requestingUserRole !== 2205) {
      if (!req.body.currentPassword || !user.hashedPassword) {
        return res
          ...
          .json({
            message:
              "Mot de passe actuel manquant ou pas de mot de passe haché enregistré.",
          });
      }
    ...
    if (requestingUserRole === 2205) {
      ...
      if (req.body.hashedPassword) {
        updatedFields.hashedPassword = await bcrypt.hash(
...
exports.setSecurityQuestion = async (req, res, next) => {
  try {
    ...
    res
      ...
      .json({
        success: true,
        message: "Question de sécurité définie avec succès.",
      });
  } catch (error) {
    next(error);
  }
...
exports.resetPasswordRequest = async (req, res, next) => {
  ...
  try {
    ...
    const securityUser = await SecuriteUtilisateur.findOne({
      where: { ID_User: user.ID_User },
    });
    if (!securityUser) {
      return next(
        new Error("Question de sécurité non trouvée pour cet utilisateur")
      );
    }
    ...
    if (!securityQuestion) {
      return res
        .status(404)
        .json({ message: "Question de sécurité non trouvée." });
    }
...
exports.resetPassword = async (req, res, next) => {
  ...
  try {
    ...
    if (!securityUser || !securityUser.Reponse_securite) {
      return next(
        new Error("Réponse de sécurité non trouvée pour cet utilisateur")
      );
    }
...
exports.deleteAccount = async (req, res, next) => {
  ...
  try {
    ...
    const groupesAdmin = await Groupe.findAll({
      where: { ID_Admin: targetUserId },
      transaction,
    });
    for (let groupe of groupesAdmin) {
      const autresMembres = await MembreGroupe.findAll({
        where: {
          ...
          ID_User: { [sequelize.Op.ne]: targetUserId },
        },
        transaction,
      });
      if (autresMembres.length > 0) {
        await groupe.update(
          { ID_Admin: autresMembres[0].ID_User },
          { transaction }
        );
      } else {
    ...
    const securityExists = await SecuriteUtilisateur.findOne({
      where: { ID_User: targetUserId },
    });
    if (securityExists) {
      await SecuriteUtilisateur.destroy({
        where: { ID_User: targetUserId },
        transaction,
      });
    }

    const historyExists = await HistoriqueChangementMotDePasse.findOne({
      where: { ID_User: targetUserId },
    });
    if (historyExists) {
      await HistoriqueChangementMotDePasse.destroy({
        where: { ID_User: targetUserId },
        transaction,
      });
    }
...
exports.getUserInfo = async (req, res, next) => {
  try {
    ...
    const user = await Utilisateur.findOne({
      where: { ID_User: targetUserId, isDeleted: false },
      attributes: {
        exclude: ["hashedPassword", "createdAt", "updatedAt", "isDeleted"],
      },
    });
...
exports.Deconnexion = async (req, res, next) => {
  try {
    ...
    if (token) {
      ...
      // Note : Si les JWT ne sont pas stockés dans la base de données, cette étape n'est pas requise
      // await ID_User.destroy({ where: { token: token } });

      // Supprimer le cookie HTTP-only en le réinitialisant avec une date d'expiration passée
      res.cookie("auth_token", "", { expires: new Date(0), httpOnly: true });

      // Envoyer une réponse indiquant la réussite de la déconnexion
      res.status(200).json({ message: "Déconnexion réussie." });
    } else {
      // Si le token est absent, envoyer une réponse indiquant que l'utilisateur est déjà déconnecté
      res
        .status(200)
        .json({
          message:
            "Aucune session active trouvée ou utilisateur déjà déconnecté.",
        });
    }
  } catch (error) {
    next(error);
